...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.vgg = vgg19[:44]
        self.prepend_model_path = lambda f: os.path.join(args.model_dir, f)
        self.vgg.load_state_dict(torch.load(self.prepend_model_path('encoder.pth'), map_location='cpu'), strict=False)
        ...
        if args.pretrained == True:
            self.align1.load_state_dict(torch.load(self.prepend_model_path('PAMA1.pth'), map_location='cpu'), strict=True)
            self.align2.load_state_dict(torch.load(self.prepend_model_path('PAMA2.pth'), map_location='cpu'), strict=True)
            self.align3.load_state_dict(torch.load(self.prepend_model_path('PAMA3.pth'), map_location='cpu'), strict=True)
            self.decoder.load_state_dict(torch.load(self.prepend_model_path('decoder.pth'), map_location='cpu'), strict=False)
    ...
    def save_ckpts(self):
        torch.save(self.align1.state_dict(), self.prepend_model_path("PAMA1.pth"))
        torch.save(self.align2.state_dict(), self.prepend_model_path("PAMA2.pth"))
        torch.save(self.align3.state_dict(), self.prepend_model_path("PAMA3.pth"))
        torch.save(self.decoder.state_dict(), self.prepend_model_path("decoder.pth"))  
